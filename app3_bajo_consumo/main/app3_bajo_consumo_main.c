/*
 * app3_bajo_consumo — Clase 3 (Sistemas Embebidos, UCU 2026)
 */
...
/* OJO: aquí NO hay ACTIVO_MS. La fase activa de esta app no tiene plazo:
 * se queda esperando toques indefinidamente (ver fase_activa()). Es el
 * cambio #2 de la consigna: "elimina el timer en el primer paso". */
...
#define I_ACTIVO_UA             23000    /* CPU a 240 MHz, ejecutando        */
#define I_LIGHT_SLEEP_UA          750    /* light sleep, VDD_SPI apagado     */
#define I_DEEP_SLEEP_UA            22    /* deep sleep con táctil al 1 %     */
/* Capacidad de la batería usada para estimar la autonomía, en µAh. */
#define BATERIA_UAH           1000000L   /* 1000 mAh */
...
static const char *TAG = "app3_bajo_consumo";
...
typedef enum {
    ...
    ACC_MEMORIA,         /* en fase activa: imprime la tabla de memoria   */
} accion_t;
 ...
 *
 * NETWORK es el botón T11 (GPIO11) de la ESP-LyraP-TouchA. Reemplaza a
 * VOL_UP del ejemplo base: sin temporizador en la fase activa, "prolongar"
 * ya no tiene sentido, así que ese canal se reutiliza para la tabla de
 * memoria. */
...
RTC_DATA_ATTR static uint32_t s_consultas_memoria;   /* veces que se pidio la tabla */
...
static void linea_modo(const char *nombre, int64_t t_ms, int32_t i_ua,
                       int64_t t_total_ms)
{
    const int64_t carga_uams = (int64_t)i_ua * t_ms;      /* µA·ms          */
    const int64_t nah = carga_uams / 3600;                /* nAh            */
...
static void informar_balance(void)
{
    ...
    ESP_LOGI(TAG, "  consultas de tabla de memoria (NETWORK): %" PRIu32,
             s_consultas_memoria);
    ESP_LOGI(TAG, "  (corrientes del datasheet, no medidas: ver el README)");
}

/* ------------------------------------------------- tabla de memoria ----- *
 * Requisito #2 de la App 3: al tocar NETWORK en fase activa se imprime esto,
 * sin salir de la fase activa. Es el mismo heap por capacidades y la misma
 * tabla de particiones de ejemplos/mapa_memoria/, pero llamado bajo demanda
 * en vez de una sola vez en el arranque. */
static void mostrar_tabla_memoria(void)
{
    s_consultas_memoria++;

    ESP_LOGI(TAG, "=== TABLA DE MEMORIA (toque NETWORK #%" PRIu32 ") ===",
             s_consultas_memoria);
    ESP_LOGI(TAG, "  %-22s %10s %10s %10s", "capacidad", "total(B)", "libre(B)", "mayor(B)");
    ESP_LOGI(TAG, "  %-22s %10u %10u %10u", "INTERNAL (SRAM)",
             (unsigned)heap_caps_get_total_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL));
    ESP_LOGI(TAG, "  %-22s %10u %10u %10u", "8BIT (byte a byte)",
             (unsigned)heap_caps_get_total_size(MALLOC_CAP_8BIT),
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_8BIT),
             (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));
    ESP_LOGI(TAG, "  %-22s %10u %10u %10u", "DMA (alcanzable por DMA)",
             (unsigned)heap_caps_get_total_size(MALLOC_CAP_DMA),
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_DMA),
             (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_DMA));
    ESP_LOGI(TAG, "  %-22s %10u %10u %10u", "RTCRAM (RTC FAST)",
    ...
    ESP_LOGI(TAG, "  margen de pila    : %u palabras de 4 B (esta tarea)",
             (unsigned)uxTaskGetStackHighWaterMark(NULL));

    ESP_LOGI(TAG, "  --- Tabla de particiones (la flash por dentro) ---");
    ESP_LOGI(TAG, "  %-10s %-6s %-8s %10s %10s", "etiqueta", "tipo", "subtipo",
             "direccion", "tamano");
    ...
    while (it != NULL) {
        ...
        ESP_LOGI(TAG, "  %-10s 0x%02x   0x%02x     0x%08" PRIx32 " %8" PRIu32 " KB",
                 p->label, p->type, p->subtype, p->address, p->size / 1024u);
        it = esp_partition_next(it);
    }
    esp_partition_iterator_release(it);

    ESP_LOGI(TAG, "=== fin tabla de memoria: seguimos en ACTIVO, sin plazo ===");
}
...
/* Fase activa: el LED verde indica "despierto". A diferencia del ejemplo
 * base, NO hay temporizador: la fase se queda aquí indefinidamente hasta que
 * alguien toca PLAY (condición de paso #1). Tocar NETWORK muestra la tabla
 * de memoria y NO hace avanzar de fase (condición de paso #2, que en
 * realidad es "no-paso": es la evidencia de que seguimos activos).
 * Devuelve los µs realmente pasados aquí. */
static int64_t fase_activa(void)
{
    ...
    led(0, 24, 0);
    ESP_LOGI(TAG, "[ACTIVO] sin plazo (sin timer) - PLAY: avanzar a light sleep "
                  "- NETWORK: tabla de memoria");

    while (1) {
        /* portMAX_DELAY: bloquea sin límite. No hay deadline que romper el
         * bucle por sí solo; solo un toque puede sacarnos de aquí. */
        ...
        if (b->accion == ACC_AVANZAR) {
            ESP_LOGI(TAG, "  condicion de paso cumplida: PLAY -> LIGHT SLEEP");
            break;
        }
        if (b->accion == ACC_MEMORIA) {
            mostrar_tabla_memoria();
            /* Sin "break": seguimos en fase activa. */
        }
    }
...
static int64_t fase_light_sleep(int ronda)
{
    ...
    if (causas & BIT(ESP_SLEEP_WAKEUP_TOUCHPAD)) {
        s_despertares_tactil++;
        ESP_LOGI(TAG, "  condicion de paso: toque, tras %" PRId64 " ms", dormido / 1000);
    } else if (causas & BIT(ESP_SLEEP_WAKEUP_TIMER)) {
        s_despertares_timer++;
        ESP_LOGI(TAG, "  condicion de paso: vencio el PLAZO tras %" PRId64 " ms",
                 dormido / 1000);
    } else {
...
static void tarea_ciclo(void *arg)
{
    while (1) {
        ...
        ESP_LOGI(TAG, "condicion de paso: %d rondas de light sleep cumplidas -> DEEP SLEEP",
                 RONDAS_LIGHT);
        informar_balance();
